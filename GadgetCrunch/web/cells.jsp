...
<%@page import="java.util.Vector, za.ac.tut.gadget.Gadget, za.ac.tut.login.Login"%>
<%
    Login login = (Login)session.getAttribute("loginObj");
    if(login != null)
    {
        if(login.getRole().equalsIgnoreCase("Client"))
        {
%>
	...
	<!-- Navigation -->
        ...
        <div class = "container">
            ...
            <header class="jumbotron hero-spacer">
                <h1 align="center" style="font-family:Arial Rounded MT Bold; text-shadow:2px 2px 4px grey;" >Cellphones</h1>  
			...
			<form action="stock.do" method="POST">
				...
				<div class="table-responsive">
					<table class="table table-striped table-hover table-border" align="center" style="background-color: #89bdff">
						...
						<%
							Vector<Gadget> phones = (Vector<Gadget>)session.getAttribute("cellphones");
						...
						%>
									<tr>
										...
										<td><%=phones.get(x).getModelCode()%></td>
										<td><%=phones.get(x).getBrand()%></td>
										<td><%=phones.get(x).getDescription()%></td>
										<td><%=phones.get(x).getCategory()%></td>
										<td><%=phones.get(x).getPrice()%></td>
										<td><%=phones.get(x).getQuantity()%></td>
                                                                               
                                                                                <td>
                                                                                    <a class="btn btn-default" href="order.do?id=<%=phones.get(x).getGadget_id()%>&action=OrderNowPhone">Order Now</a>
                                                                                </td>
									</tr>
						<%
								}
...
<%
        ...
        {
%>
<jsp:forward page="login_as_client.html"/>
<%
        }
    }
%>